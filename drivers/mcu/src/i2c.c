...
STATUS_CODE I2C_Init(I2C_TypeDef* I2Cx, uint16_t mode, uint16_t duty_cycle, uint32_t clock_speed) {
    ...
    uint32_t pclk1_mhz = APB1_GetPCLKFreq() / 1000000;
...
STATUS_CODE I2C_ClockConfig(I2C_TypeDef* I2Cx, uint16_t mode, uint16_t duty_cycle, uint32_t clock_speed) {
    ...
    uint32_t pclk1 = APB1_GetPCLKFreq(); // Get APB1 clock frequency 
    uint32_t ccr_val = 0;

    // Set the CCR value based on the desired clock speed and mode
    switch (mode) {
        case I2C_SLOW_MODE: // Standard mode (100 kHz)
...
STATUS_CODE I2C_Start(I2C_TypeDef* I2Cx) {
    ...
    while ((I2Cx->SR2 & I2C_BUSY) && --timeout);
    if (timeout == 0) {
...
STATUS_CODE I2C_SendAddress(I2C_TypeDef* I2Cx, uint8_t slave_address, uint8_t direction) {
    ...
    // Wait for ADDR flag
    uint32_t timeout = TIMEOUT;
    while (!(I2Cx->SR1 & I2C_ADDR) && --timeout);
    if (timeout == 0) {
...
STATUS_CODE I2C_Transmit(I2C_TypeDef* I2Cx, uint8_t* data, uint16_t size) {
    ...
    for (uint16_t i = 0; i < size; i++) {
        ...
        // Wait for TXE flag
        timeout = TIMEOUT;
        while (!(I2Cx->SR1 & I2C_TXE) && --timeout);
        if (timeout == 0) {
    ...
    timeout = TIMEOUT;
    while (!(I2Cx->SR1 & I2C_BTF) && --timeout);
    if (timeout == 0) {
...
STATUS_CODE I2C_Receive(I2C_TypeDef* I2Cx, uint8_t* data, uint16_t size) {
    ...
    if (size == 1) {
        ...
        I2Cx->CR1 &= ~I2C_ACK; // Disable ACK for last byte
        
        timeout = TIMEOUT;
        while (!(I2Cx->SR1 & I2C_RXNE) && --timeout); // Wait for RXNE
        if (timeout == 0) {
        ...
        I2Cx->CR1 |= I2C_ACK; // Enable ACK for multiple bytes
        I2Cx->CR1 |= I2C_POS; // Set POS

        timeout = TIMEOUT;
        while(!(I2Cx->SR1 & I2C_BTF) && --timeout); // Wait for first byte
        if (timeout == 0) {
            return STATUS_TIMEOUT; // Data not received
        }
        ...
        for (uint16_t i = 0; i < size - 3; i++) {
            timeout = TIMEOUT;
            while(!(I2Cx->SR1 & I2C_RXNE) && --timeout); // Wait for first byte
            if (timeout == 0) {
                return STATUS_TIMEOUT; // Data not received
            }
        ...
        I2Cx->CR1 &= ~I2C_ACK; // Set ACK low
        data[size - 3] = I2Cx->DR; // Read data N-2
        
        timeout = TIMEOUT;
        while (!(I2Cx->SR1 & I2C_BTF) && --timeout); // Wait for second to last byte
        if (timeout == 0) {
            return STATUS_TIMEOUT; // Data not received
        }

        I2Cx->CR1 |= I2C_STOP; // Generate STOP condition after reading second to last byte
